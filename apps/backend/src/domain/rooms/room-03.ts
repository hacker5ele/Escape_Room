 ...
 * frontend (see ADR-0023, ADR-0027). Losing the last one anywhere sends the
 ...
 * log" approach ADR-0022 established, still tracking hearts (ADR-0023).
 *
...
export const room03: RoomDefinition = {
  ...
  // complete via POST /api/rooms/:roomId/complete. See ADR-0027.
  canComplete(session) {
  ...
  publicData(session) {
    ...
    return {
      ...
      // Olympus, both client-only from here — see ADR-0023, ADR-0027) can
  ...
  check(answer, session) {
    ...
    if (choiceIndex === riddle.correct) {
      ...
      return {
        ...
        // POST /api/rooms/:roomId/complete (ADR-0027) actually finishes the
