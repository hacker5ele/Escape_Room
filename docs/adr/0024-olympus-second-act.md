# ADR-0024: A second riddle act — Mount Olympus — after the Atlantis quest

- **Status:** Proposed
- **Date:** 2026-08-05
- **Deciders:** Inaam Ahmed
- **Approved-by:** _(pending — awaiting Nepomuk Crhonek's review)_
- **Superseded by:** [ADR-0027](0027-olympus-carpet-race.md)

> **Superseded 2026-08-05.** Olympus is a carpet-racing coin challenge, not a riddle sequence — see
> ADR-0027. `OLYMPUS_RIDDLES` and the two-act (`sphinx` | `olympus`) split described below no longer
> exist in `room-03.ts`. This document is kept for the reasoning trail (in particular, the event-log
> replay technique it built on top of, which ADR-0025 and ADR-0027 both still rely on for the Sphinx's
> own five riddles).
...
Two things had to be decided, both interface-shaped for the same reason ADR-0022 and ADR-0023 already
...
Olympus's own riddle 1.** This matches "you lost this run" from ADR-0023: Olympus is only reachable by
...
- A player who reaches Olympus, then opens a second tab and replays the Sphinx from scratch in it, could
  ...
  ADR-0023 (Atlantis's own local-vs-server drift under a second tab), not worth solving for.
- `apps/frontend/src/rooms/preview.tsx`'s mock riddle set was updated to match the new Sphinx riddle
  ...
  (documented limitation, unchanged from ADR-0022).
