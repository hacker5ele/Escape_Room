# ADR-0070: Olympus becomes a carpet-racing coin challenge, not a riddle sequence
...
ADR-0067 built Olympus as a second server-checked riddle act, mirroring the Sphinx's five questions.
...
This has real consequences for room-03's backend, which ADR-0067/ADR-0068 had already shaped around a
...
never sees an answer for at all, so there is no `check()` call left for `roomComplete` (ADR-0068) to
...
point; per ADR-0069, that stays reserved for the waiting room's own "Finish" button, so the app doesn't
...
the same severity as an Atlantis timeout, using the identical shared-hearts mechanism (ADR-0066). Losing
...
- ADR-0067's two-act riddle design is superseded for the Olympus stage specifically — its
  event-log-replay technique (deriving progress from the event log, ADR-0065's original idea) remains
...
- `POST /api/rooms/:roomId/complete` is generic by room id even though only room-03 uses it today, same
  reasoning as `/reset` (ADR-0066): any future room whose final stage is client-side gets this for free.
...
- `apps/frontend/src/rooms/preview.tsx`'s mock gained a no-op `onCompleteRoom`, same treatment as
  `onRoomFinished` got in ADR-0069 — the preview never calls it for real.
...
reason ADR-0066 rejected a similar trick for Atlantis: it would log a fake answer for a challenge that
