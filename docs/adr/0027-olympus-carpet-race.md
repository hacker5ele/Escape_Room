# ADR-0027: Olympus becomes a carpet-racing coin challenge, not a riddle sequence
...
ADR-0024 built Olympus as a second server-checked riddle act, mirroring the Sphinx's five questions.
...
This has real consequences for room-03's backend, which ADR-0024/ADR-0025 had already shaped around a
...
never sees an answer for at all, so there is no `check()` call left for `roomComplete` (ADR-0025) to
...
point; per ADR-0026, that stays reserved for the waiting room's own "Finish" button, so the app doesn't
...
the same severity as an Atlantis timeout, using the identical shared-hearts mechanism (ADR-0023). Losing
...
- ADR-0024's two-act riddle design is superseded for the Olympus stage specifically — its
  event-log-replay technique (deriving progress from the event log, ADR-0022's original idea) remains
...
- `POST /api/rooms/:roomId/complete` is generic by room id even though only room-03 uses it today, same
  reasoning as `/reset` (ADR-0023): any future room whose final stage is client-side gets this for free.
...
- `apps/frontend/src/rooms/preview.tsx`'s mock gained a no-op `onCompleteRoom`, same treatment as
  `onRoomFinished` got in ADR-0026 — the preview never calls it for real.
...
reason ADR-0023 rejected a similar trick for Atlantis: it would log a fake answer for a challenge that
