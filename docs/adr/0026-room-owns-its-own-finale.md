# ADR-0026: A room decides when it is done showing its own finale
...
displayed — the moment they differed (because the last attempt made the room `roomComplete`, ADR-0025),
...
This is a second, distinct bug from ADR-0025's — that one was about *when the server* considers a room
...
- Every room that gets a real frontend component from here on must remember to call `onRoomFinished`.
  ...
  remember, the same way ADR-0025 made `roomComplete` the one thing a multi-stage room's `check()` must
...
- `apps/frontend/src/rooms/preview.tsx`'s mock `RoomProps` gained a no-op `onRoomFinished` to keep
  ...
  advancement at all.
- Room 3's actual finale, once this and ADR-0025 were both fixed, is: Sphinx's five riddles → Atlantis
